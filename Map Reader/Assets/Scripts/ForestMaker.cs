...
public class ForestMaker : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        {
            if (way.typeOfNatural == "wood" || way.typeOfNatural == "scrub")
            {
                ...
                terrainMaterial = Resources.Load<Material>("Materials/" + way.typeOfTerrain);
                if(terrainMaterial==null)
                    terrainMaterial = Resources.Load<Material>("Materials/default");
    ...
    void SpawnTrees(float x, float z)
    {
        for (int i = 0; i < 10; i++)
        {
            Vector3 position = new Vector3(x+rnd.Next(0,5),0,z+rnd.Next(0,5));
            position.y = GetHeight(position.x, position.z);
            if(position.y<0)
                continue;
            Instantiate(trees[rnd.Next(0, trees.Length - 1)], position,
                Quaternion.LookRotation(Vector3.right, Vector3.up));
        }
    }
    void AddtoMesh(Vector3 v1, Vector3 v2, Vector3 v3)
    {
        SpawnTrees(v1.x,v1.z);
        SpawnTrees(v2.x,v2.z);
        SpawnTrees(v3.x,v3.z);
        vectors.Add(v1);
        vectors.Add(v2);
        vectors.Add(v3);
        vectors.Add(v1);
