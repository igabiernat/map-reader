...
class RiverMaker : MonoBehaviour
{
    public Material riverMaterial;
    public MeshCollider mc;
    private MapReader map;
    private float riverWidth = 10f;
            ...
            if (way.isRiver)
            {
                ...
                for (int i = 1; i < way.childrenIDs.Count; i++)
                {
                    ...
                    if (vectors.Count > 2)
                    {
                        ...
                        var crossfirst = Vector3.Cross(difffirst, Vector3.up) * riverWidth;

                        Vector3 v1first = n1 + crossfirst;
                        Vector3 v2first = n1 - crossfirst;

                        v1first.y = GetHeight(v1first.x, v1first.z)+0.6f;
                        v2first.y = GetHeight(v2first.x, v2first.z)+0.6f;
                    ...
                    for (int j = 1; j <= additionalvertices; j++)
                    {
                        ...
                        var cross = Vector3.Cross(diff, Vector3.up) * riverWidth;
                        ...
                        v1.y = GetHeight(v1.x, v1.z)+0.6f;
                        v2.y = GetHeight(v2.x, v2.z)+0.6f;
                        v3.y = GetHeight(v3.x, v3.z)+0.6f;
                        v4.y = GetHeight(v4.x, v4.z)+0.6f;
                ...
                /*if(vectors.Count > 2)
                    if ((vectors[0] - vectors[vectors.Count - 1]).magnitude < 5f)
                    {
